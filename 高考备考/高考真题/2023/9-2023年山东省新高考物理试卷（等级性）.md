...
因此
$$
\varphi_{A^{\prime}}-\varphi_{F^{\prime}}=\varphi-\varphi_{F^{\prime}}\leq \varphi_{O^{\prime}}-\varphi_{D^{\prime}}=\varphi
$$
...
$f=\mu mg=0.2\times 1\times 10N=2N$
...
(2)调节滑动变阻器滑片位置，电压表表盘如图乙所示，示数为 $6.5V$ (保留1位小数)。
...
16. 一种反射式光纤位移传感器可以实现微小位移测量，其部分原理简化如图所示。两光纤可等效为圆柱状玻璃丝 $M、N$ ，相距为 $d$ ，直径均为 $2a$ ，折射率为 $n(n\leq \sqrt{2})$ 。 $M、N$ 下端横截面平齐且与被测物体表面平行。激光在 $M$ 内多次全反射后从下端面射向被测物体，经被测物体表面镜面反射至 $N$ 下端面， $N$ 下端面被照亮的面积与玻璃丝下端面到被测物体距离有关。
...
(2)滑块C刚滑上B时，对C受力分析，由牛顿第二定律得： $\mu _2m_Cg=m_Ca_C$
