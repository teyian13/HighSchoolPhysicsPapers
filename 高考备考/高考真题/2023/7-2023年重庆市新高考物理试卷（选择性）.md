...
B. $FM$ 段无人机的货物处于失重状态
C. $FN$ 段无人机和装载物总动量变化量大小为 $4\mathrm{~kg}\cdot \mathrm{~m/s}$
D. $MN$ 段无人机机械能守恒
【答案】AB
...
C、从 $MN$ 的直线方向 $y=-2t+140$ 可知， $F$ 点的速度 $v_F=-2m/s$ ，所以 $FN$ 段的动量变化： $\Delta p_{FN}=mv_N-mv_F=2\times (-2)\mathrm{~kg}\cdot \mathrm{~m/s}-2\times 4\mathrm{~kg}\cdot \mathrm{~m/s}=-12\mathrm{~kg}\cdot \mathrm{~m/s}$ ，故C错误；
...
(2)用摆线和摆球组成单摆，如图1乙所示。当摆线长度 $l=990.1\mathrm{~mm}$ 时，记录并分析单摆的振动视频，得到单摆的振动周期 $T=2$ . $00\mathrm{~s}$ ，由此算得重力加速度 $g$ 为 $9.86\mathrm{~m/s}^2$ （保留3位有效数字）。
