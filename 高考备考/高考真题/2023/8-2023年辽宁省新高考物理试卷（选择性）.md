...
【答案】A
【解答】因原子能级跃迁放出的光子的能量等于原子的能级差，由题图可知光子(1)、(2)、(3)、(4)对应的能量关系为： $E_{(2)}\leq E$ (1) $=E_{(3)}\leq E_{(4)}$
A、由上述分析可知光子(1)和(3)的能量相等，故A正确；
B、由光子能量 $E=hv$ ，和 $E_{(2)}\leq E_{(4)}$ ，可得到(2)的频率小于(4)的频率，故B错误；
C、发生光电效应的条件是光子的能量大于金属的逸出功，已知用(1)照射该金属表面时能发生光电效应，可知 $E_{(1)}大$ 此金属的逸出功 $W_0$ ，因 $E_{(2)}\leq E_{(1)}$ ，则无法比较 $E_{(2)}$ 与 $W_0$ 的大小关系，故用(2)照射该金属不一定能发生光电效应，故C错误；
